    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-luxury-charcoal h-9 flex items-center overflow-hidden">
        <div className="flex animate-marquee w-full whitespace-nowrap">
          {Array(3)
            .fill(
              "FREE SHIPPING ON ORDERS ABOVE ₹4,999  ·  NEW COLLECTION NOW LIVE  ·  ",
            )
            .map((text, i) => (
              <span
                key={i}
                className="text-luxury-white text-xs font-sans font-200 tracking-widest mr-0"
              >
                {text}
              </span>
            ))}
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center gap-12">
              ...
              >
                ...
                {wishlistCount > 0 && (
                  <span
                    className="absolute top-0 right-0 w-3 h-3 rounded-full bg-luxury-gold text-luxury-white text-xs flex items-center justify-center"
                  ...
                  <AnimatePresence>
                    {userMenuOpen && (
                      ...
                      >
                        <Link
                          to="/account"
                          className="flex items-center gap-2 px-4 py-2 text-sm text-luxury-black hover:bg-luxury-cream transition-colors"
                        >
                        ...
                        {user.role === "admin" && (
                          <Link
                            to="/admin"
                            className="flex items-center gap-2 px-4 py-2 text-sm text-luxury-gold hover:bg-luxury-cream transition-colors font-500"
                          >
                            Admin Panel
                          </Link>
                        )}
        ...
        {mobileOpen && (
          <>
            ...
            >
              <Link
                to="/"
                className="font-garamond-serif italic text-3xl font-300 mb-12"
                style={{ color: "var(--color-black)", letterSpacing: "0.3em" }}
              >
                VELOUR
              </Link>
              ...
              <div className="mt-auto space-y-4">
                {!user ? (
                  <Link
                    to="/login"
                    className="block w-full bg-luxury-gold text-luxury-white py-3 text-center text-sm font-400 tracking-widest uppercase"
                  >
                    LOGIN
                  </Link>
                ) : (
                  <>
                    <Link
                      to="/account"
                      className="block text-luxury-black text-sm font-400 tracking-widest uppercase hover:text-luxury-gold transition-colors"
                    >
                      MY ACCOUNT
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-2 text-red-500 font-500 text-sm"
                    >
